...
News ingestion runs **every 10 minutes**. Curated RSS feeds are pulled on every run, while the Google News (Serper) queries are throttled to roughly once an hour to stay inside the provider's daily quota. The pipeline:
